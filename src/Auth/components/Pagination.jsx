...
export const Pagination = ({ totalPosts, postPerPage, setCurrentPage, currentPage}) => {
  ...
  return (
    <div className='w-full flex items-center justify-between py-4 px-5 bg-bgc_white-100 dark:bg-dark-800 dark:border-gray-700 border-t border-gray-300 h-24'>
        ...
        > <HiChevronLeft /> Anterior</button>
            <div className='flex gap-3'>
                ...
                <form onSubmit={ onSubmitPage }>
                    <input type="number" className='outline-none py-1 px-2 rounded-lg text-gray-600 bg-white dark:text-gray-300 dark:bg-dark-700' min="1" max={ Math.ceil(totalPosts/postPerPage)} onChange={ handlePage } value={ pageValue } />
                </form>
        ...
        > Siguiente <HiChevronRight /> </button>
    </div>
  )
}
